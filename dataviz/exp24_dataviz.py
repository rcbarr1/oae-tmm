...
--> total delDIC / total delAT at 5 and 15 years
...
for experiment_name in tqdm(experiment_names):
    try:
        with xr.open_mfdataset(
            ...
            # convert delDIC and delAT from µmol kg-1 to mol, sum over ocean volume
            delDIC = (ds.delDIC * rho * model_vols_xr * 1e-6).sum(dim=['lat', 'lon', 'depth'], skipna=True)
            delAT  = (ds.delAT  * rho * model_vols_xr * 1e-6).sum(dim=['lat', 'lon', 'depth'], skipna=True)
...
cntr0 = axs[0].contourf(model_lon_rot, model_lat, nu_5years_masked,  levels=levels, cmap='viridis', vmin=vmin, vmax=vmax)
cntr1 = axs[1].contourf(model_lon_rot, model_lat, nu_15years_masked, levels=levels, cmap='viridis', vmin=vmin, vmax=vmax)
...
for t_idx in tqdm(range(0, len(ds.time.values))):
    plot_surface2d(model_lat, model_lon, ds.delDIC.isel(time=t_idx, depth=0).values + 0.0001, 0, 0.1, 'viridis', 'delDIC at t = ' + str(ds.time.isel(time=t_idx).values))
    # plot_longitude3d(model_lat, model_depth, ds.delAT.isel(time=t_idx)+0.0001, AT_lon, 0, 10, 'viridis', 'delAT at t = ' + str(ds.time.isel(time=t_idx).values))

...
if interp:
    for scenario in scenarios:
        ...
        for idx in tqdm(range(len(t))):
            if scenario != 'none':
                Canth_idx_3D = p2.interp_TRACE(data_path, t[idx], scenario, model_lat, model_lon, model_depth, ocnmask)
            else:
...
# without interpolation
    ...
    for scenario in scenarios:
        ...
        for idx in tqdm(range(len(t))):
            if scenario != 'none':
                Canth_idx_3D = p2.calculate_canth(scenario, t[idx], T_3D, S_3D, ocnmask, model_lat, model_lon, model_depth)
            else:
                Canth_idx_3D = Canth_3D
            Canth_all_idx.append(p2.flatten(Canth_idx_3D, ocnmask))
        Canth_all_scenarios.append(Canth_all_idx)
...
for exp_idx in range(1):
    ...
    for idx in np.arange(0, len(ds.time), 12):
        ...
        p2.plot_surface3d(model_lat, model_lon, delpH, 0, -0.2, 0.2, 'RdBu', 'delpH in ' + str(ds['time'].isel(time=idx).values))
...
for exp_idx in range(len(experiment_names)):
    ...
    for idx in tqdm(range(len(ds['time']))):
        ...
        Canth_ds = xr.DataArray(p2.make_3D(Canth_all_scenarios[exp_idx][idx], ocnmask), dims=["lat", "lon", "depth"], coords={"lat": ds.lat, "lon": ds.lon, "depth": ds.depth})
...
p2.make_surf_animation_pH(pH_modeled, 'Deviation of pH from preindustrial', model_lat, model_lon, ds.time.values, len(ds.time.values), ocnmask, -0.2, 0.2, 'RdBu', 'exp22_2026-02-06_t1_none_pH_deviation_preind.mp4')
